...
def run(model_names_for_mining, target_model_name, source_lang, target_lang, task1_name, task2_name, code_dir, transcode_dir, transcode_script_dir, path_to_save_map_for_line, path_to_save_map_for_statement, path_to_save_unmapped_stmt):
    ...
    save_unmapped_stmt_dir = f'{path_to_save_unmapped_stmt}/{target_model_name}-{source_lang}-{target_lang}'
    os.makedirs(save_unmapped_stmt_dir, exist_ok=True)
    ...
    for ID in IDs[:]:
        ...
        for ori_id, ori_path in tqdm(enumerate(source_stmt_list)):
            ...
            possible_maps_list = [this_item[0] for this_item in possible_maps_list]
            if not possible_maps_list:
                ...
                if search:
                    ...
                    possible_maps_force_list = [this_item[0] for this_item in possible_maps_force_list]
                else:
                    possible_maps_force_list, _ = match_force(this_source_path2tree[ori_path], [], root_node2map,
                                                                    ...
                                                                    max_depth,
                                                                    max_possible_choices, start_time, time_limit, False, path_path2anchors)
                    possible_maps_force_list = remove_parenthesis(possible_maps_force_list)
                    save_new_rule(f'{task1_name}-Synthesis', ori_path, possible_maps_force_list)
                    possible_maps_force_list = [this_item[0] for this_item in possible_maps_force_list]
                possible_maps_list.extend(possible_maps_force_list)
            possible_maps_list = rephrase_maps_for_tree([[item] for item in possible_maps_list], target_lang)
            possible_maps_list = [item[0] for item in possible_maps_list]
        ...
        for ori_id, ori_path in tqdm(enumerate(trans_stmt_list)):
            if ori_id == 0 or ori_path == 'expression_statement-0||||;-0':
                continue
            ...
            time_limit = 1000000000
            start_time = time.time()
            possible_maps_list = match(this_trans_path2tree[ori_path], [], target_lang, source_lang, inverse_maps, inverse_source_path2tree, inverse_path_path2anchors)
            possible_maps_list = [this_item[0] for this_item in possible_maps_list]
            if not possible_maps_list:
                search, search_result = check_new_rule(f'{task2_name}-Synthesis', ori_path)
                if search:
                    possible_maps_force_list = search_result
                    possible_maps_force_list = [this_item[0] for this_item in possible_maps_force_list]
                else:
                    possible_maps_force_list, _ = match_force(this_trans_path2tree[ori_path], [], inverse_root_node2map,
                                                                    ...
                                                                    inverse_maps, inverse_source_path2tree, inverse_trans_path2tree, depth,
                                                                    max_depth,
                                                                    max_possible_choices, start_time, time_limit, False, inverse_path_path2anchors)
                    possible_maps_force_list = remove_parenthesis(possible_maps_force_list)
                    save_new_rule(f'{task2_name}-Synthesis', ori_path, possible_maps_force_list)
                    possible_maps_force_list = [this_item[0] for this_item in possible_maps_force_list]
                possible_maps_list.extend(possible_maps_force_list)
            possible_maps_list = rephrase_maps_for_tree([[item] for item in possible_maps_list], source_lang)
            possible_maps_list = [item[0] for item in possible_maps_list]
            if possible_maps_list:
                count3 += 1
            ...
            if not if_s_matched:
                for template_list_id, template_list in enumerate(templates):
                    for trans_id in range(len(trans_stmt_list)-len(template_list)+1):
                        if_matched = True
                        for this_id in range(len(template_list)):
                            this_diff = compare_MyTree_more(template_list[this_id], this_trans_trees[trans_id+this_id], target_lang)
                            if this_diff:
                                if_matched = False
                                break
                        if if_matched:
            ...
            if not if_t_matched:
                for template_list_id, template_list in enumerate(templates):
                    for source_id in range(len(source_stmt_list)-len(template_list)+1):
                        if_matched = True
                        for this_id in range(len(template_list)):
                            this_diff = compare_MyTree_more(template_list[this_id], this_source_trees[source_id+this_id], source_lang)
                            if this_diff:
                                if_matched = False
                                break
                        if if_matched:
        ...
        new_new_updated_M = extend_maps(new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables)
        ...
        for t_stmt_id, t_stmt in enumerate(trans_stmt_list):
            if t_stmt_id:
                if_t_have_rule = False
                if t_stmt in trans_stmt2mapping:
                    for item in trans_stmt2mapping[t_stmt]:
                        if item:
                            if_t_have_rule = True
                if_ori_mapped = False
                for k, v in M.items():
                    if v and k != '0-0':
                        if k.split('-')[1] == str(t_stmt_id):
                            if_ori_mapped = True
                new_mapped_s_ids = []
                if_new_mapped = False
                for k, v in new_new_updated_M.items():
                    if v and k != '0-0':
                        if k.split('-')[1] == str(t_stmt_id):
                            if_new_mapped = True
                            if source_stmt2mapping[source_stmt_list[int(k.split('-')[0])]]:
                                new_mapped_s_ids.append(int(k.split('-')[0]))
                if if_new_mapped and not if_ori_mapped and (new_mapped_s_ids != [] or if_t_have_rule):
                    if t_stmt_id not in unmapped_line_ids:
                        unmapped_line_ids.append(t_stmt_id)
        ...
        f_map = open(f'{save_unmapped_stmt_dir}/{ID}.txt', 'w')
        for i in unmapped_line_ids:
            print(i, file=f_map)
        f_map.close()
...
if __name__ == "__main__":
    ...
    path_to_save_unmapped_stmt = args.path_to_save_unmapped_stmt
    ...
    count_right, count_wrong, count_right_B, count_wrong_B = run(model_names_for_mining, target_model_name, source_lang, target_lang, task1_name, task2_name, code_dir, transcode_dir, transcode_script_dir, path_to_save_map_for_line, path_to_save_map_for_statement, path_to_save_unmapped_stmt)