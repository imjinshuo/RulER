    ...
    for method_name in method_names:
        for target_model_name in target_model_names:
            for source_lang in source_langs:
                S_locate = run(method_name, path_to_DATABASE, target_model_name, source_lang, target_lang)
                S_locates[method_name].append(S_locate)
    for method_name in method_names:
        print(f'{method_name} average S_locate: {sum(S_locates[method_name])/len(S_locates[method_name])}')
