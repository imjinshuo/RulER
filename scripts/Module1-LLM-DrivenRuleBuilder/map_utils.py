...
def build_statement_mapping(model_names_for_mining, source_lang, target_lang, task_name, datasets, number1):
    maps = {}
    ori_path2tree = {}
    path2tree = {}
                    ...
                    if info_line.strip():
                        ...
                        if source_lang == 'Java':
                            ...
                            ori_source_stmt_info_lists1 = traverse_tree(source_tree1, source_lang, this_source_lines1,
                                                                       ...
                                                                       fun_block=0)
                            ori_source_stmt_info_lists1 = [item for item in ori_source_stmt_info_lists1 if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                            ...
                            ori_source_stmt_info_lists2 = traverse_tree(source_tree2, source_lang, this_source_lines2,
                                                                       ...
                                                                       exclude_last_child=False, only_path=True,
                                                                       fun_block=0)
                            ori_source_stmt_info_lists2 = [item for item in ori_source_stmt_info_lists2 if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                        ...
                        if target_lang == 'Java':
                            ...
                            ori_trans_stmt_info_lists1 = traverse_tree(trans_tree1, target_lang, this_trans_lines1,
                                                                      ...
                                                                      exclude_last_child=False, only_path=True,
                                                                      fun_block=0)
                            ori_trans_stmt_info_lists1 = [item for item in ori_trans_stmt_info_lists1 if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                            ...
                            ori_trans_stmt_info_lists2 = traverse_tree(trans_tree2, target_lang, this_trans_lines2,
                                                                      ...
                                                                      exclude_last_child=False, only_path=True,
                                                                      fun_block=0)
                            ori_trans_stmt_info_lists2 = [item for item in ori_trans_stmt_info_lists2 if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                        ...
                        source_predecessors, source_successors, source_stmt_use_consts, source_stmt_def_variables, source_stmt_use_variables, source_line_def_variables = parse_vari_dep(
                            source_stmt_list1, source_code_lines1, source_stmt_list_pos1, source_lang, this_source_trees1)
                        trans_predecessors, trans_successors, trans_stmt_use_consts, trans_stmt_def_variables, trans_stmt_use_variables, trans_line_def_variables = parse_vari_dep(
                            trans_stmt_list1, trans_code_lines1, trans_stmt_list_pos1, target_lang, this_trans_trees1)
                        ...
                        if original_path.startswith('return'):
                            ...
                            if len(this_trans_filter_more) == 1 and this_trans_filter_more[0].startswith('for_statement'):
                                this_source_filter_more_def_vs = source_stmt_def_variables[source_more_stmt_filt[0][1]]
                                this_trans_filter_more_def_vs = trans_stmt_def_variables[this_trans_filter_more_id[0]]
                                if this_trans_filter_more_id[0]+1 in trans_stmt_def_variables:
                                    all_trans_stmt_def_vs = this_trans_filter_more_def_vs[:]
                                    all_trans_stmt_def_vs.extend(trans_stmt_def_variables[this_trans_filter_more_id[0]+1])
                                    if set(this_source_filter_more_def_vs) != set(this_trans_filter_more_def_vs) and set(this_source_filter_more_def_vs) == set(all_trans_stmt_def_vs):
                                        this_trans_filter_more.append(trans_stmt_list1[this_trans_filter_more_id[0]+1])
                                        this_trans_filter_more_id.append(this_trans_filter_more_id[0]+1)
                            if not this_trans_filter_more_id:
                                continue
                            if this_trans_filter_less:
                                continue
                            if this_trans_filter_more:
                                if not validate_map([original_path, this_trans_filter_more], source_lang, target_lang):
                                    continue
...
def extend_mapping1(model_names_for_mining, source_lang, target_lang, task_name, datasets, loop_limit, number1):
    max_loop = -1
            ...
            for model_name in this_model_names_for_mining:
                ...
                for file_ID in tqdm(IDs):
                    ...
                    if source_lang == 'Java':
                        ...
                        ori_source_stmt_info_lists = [item for item in ori_source_stmt_info_lists if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                    ...
                    if target_lang == 'Java':
                        ...
                        ori_trans_stmt_info_lists = [item for item in ori_trans_stmt_info_lists if not (item[1].text.strip().startswith('print') and 'RulEROUTPUT:' in item[1].text.strip())]
                    ...
                    if dataset in ['GeeksforGeeks']:
                        ...
                        if not check_ERROR_map(this_new_map):
                            continue
                        if not validate_map(this_new_map, source_lang, target_lang):
                            continue
                    ...
                    for t_stmt_id, t_stmt in enumerate(trans_stmt_list):
                        ...
                        if len(t_def_vars) == 0:
                            continue
                        if_def_first = True
                        for def_var in t_def_vars:
                            this_def_ids = []
                            for t_id in range(len(trans_stmt_list)):
                                if def_var in trans_stmt_def_variables[t_id]:
                                    this_def_ids.append(t_id)
                            this_def_ids.sort()
                            if this_def_ids[0] != t_stmt_id:
                                if_def_first = False
                        if not if_def_first:
                            continue
                        for possible_mapped_s_id, s_stmt in enumerate(source_stmt_list):
                            s_def_vars = set(source_stmt_def_variables[possible_mapped_s_id]) if possible_mapped_s_id in source_stmt_def_variables else set([])
                            s_use_vars = set(source_stmt_use_variables[possible_mapped_s_id]) if possible_mapped_s_id in source_stmt_use_variables else set([])
                            if len(s_def_vars) == 0:
                                continue
                            if_def_first = True
                            for def_var in s_def_vars:
                                this_def_ids = []
                                for s_id in range(len(source_stmt_list)):
                                    if def_var in source_stmt_def_variables[s_id]:
                                        this_def_ids.append(s_id)
                                this_def_ids.sort()
                                if this_def_ids[0] != possible_mapped_s_id:
                                    if_def_first = False
                            if not if_def_first:
                                continue
                            if t_def_vars and s_def_vars and t_def_vars == s_def_vars and t_use_vars == s_use_vars:
...
def extend_mapping2(model_names_for_mining, source_lang, target_lang, task_name, datasets, loop_limit, number1):
    max_loop = -1
...
def build_expression_mapping(model_names_for_mining, source_lang, target_lang, task_name):
    max_loop = -1
    existing_maps_files_number = [int(file.split('.')[-2].split('-')[-1]) for file in os.listdir(f'{task_name}/') if
                                  file.startswith(
                                      f'{"_".join(model_names_for_mining)}-{source_lang}-{target_lang}-maps-') and
    ...
    for loop_time in trange(max_loop, 1000):
            ...
            for pair_ID, pair in enumerate(pairs):
                ...
                for target_id, trans_tree in enumerate(pair.trans_trees):
                    ...
                    for this_trans_subtree in this_trans_subtrees:
                        trans_subtrees.append([this_trans_subtree, target_id])
                ...
                for s_path_tree in filt_source_subtrees:
                    for t_path_tree in filt_trans_subtrees:
                        if len(s_path_tree[1].children) and len(t_path_tree[0][1].children):
                            s_children_anchors = []
                            for s_path_tree_child_id, s_path_tree_child in enumerate(s_path_tree[1].children):
                                this_s_child_path = s_path_tree[0][:]
                                this_s_child_path.append(s_path_tree_child_id)
                                for anchor in update_source_path2pair[source_path][pair_ID].anchors:
                                    if this_s_child_path == anchor[0]:
                                        s_children_anchors.append(anchor[1])
                            if_s_children_anchors_matches = True
                            if not s_children_anchors:
                                if_s_children_anchors_matches = False
                                break
                            else:
                                for s_children_anchor in s_children_anchors:
                                    if_s_children_anchors_match = False
                                    for this_s_children_anchor in s_children_anchor:
                                        this_s_children_anchor_path = this_s_children_anchor[0]
                                        if len(this_s_children_anchor_path) > 1 and this_s_children_anchor[1] == t_path_tree[1] and this_s_children_anchor_path[:-1] == t_path_tree[0][0]:
                                            if_s_children_anchors_match = True
                                            break
                                    if not if_s_children_anchors_match:
                                        if_s_children_anchors_matches = False
                                        break
                            if if_s_children_anchors_matches:
                                s_subtrees = [s_path_tree]
                                t_subtree = t_path_tree
                                if_add = update_source_path2pair[source_path][pair_ID].addAnchor([s_subtrees[0][0], t_subtree[0][0], t_subtree[1]])
                                if if_add:
                                    new_anchor_count += 1
                                    if len(pair.target_paths) == 1:
                                        ...
                                        if this_new_map[0] not in update_maps:
                                            new_maps_count += 1
                                            update_maps[this_new_map[0]] = [this_new_map[1]]
                                            maps2trees[this_new_map[0] + '>>>>' + '####'.join(this_new_map[1])] = [source_child_tree, [trans_child_tree]]
                                            ...
                                            logout_subtree(task_name, source_lang, target_lang, new_maps_count,
                                                           ...
                                                           pair.source_path, pair.target_paths[0],
                                                           pair.source_tree.text, pair.trans_trees[0].text, loop_time)
                                        source_path1 = pair.source_path
                                        target_path1 = pair.target_paths[0]
                                        source_diff_path_type = [s_subtrees[0][0]]
                                        target_diff_path_type = [t_subtree[0][0]]
                                        if source_path1 in update_source_path2pair:
                                            for pair_item in update_source_path2pair[source_path1]:
                                                if pair_item.target_paths == [target_path1]:
                                                    pair_item_anchors = pair_item.anchors
                                                    path_s = source_diff_path_type[0]
                                                    path_t = target_diff_path_type[0]
                                                    record_anchors = []
                                                    for pair_item_anchor in pair_item_anchors:
                                                        s_pair_item_anchor = pair_item_anchor[0]
                                                        t_pair_item_anchor = pair_item_anchor[1][0][0]
                                                        if len(s_pair_item_anchor) > len(path_s) and s_pair_item_anchor[:len(path_s)] == path_s and len(t_pair_item_anchor) > len(path_t) and t_pair_item_anchor[:len(path_t)] == path_t:
                                                            record_anchors.append([s_pair_item_anchor[len(path_s):], [[t_pair_item_anchor[len(path_t):], 0]]])
                                                    if this_new_map[0] in update_source_path2pair:
                                                        mapped_pairs = update_source_path2pair[this_new_map[0]]
                                                        if_contained = False
                                                        for mapped_pair_id, mapped_pair in enumerate(mapped_pairs):
                                                            if mapped_pair.target_paths == this_new_map[1]:
                                                                if_contained = True
                                                        if not if_contained:
                                                            new_path2pair_count += 1
                                                            update_source_path2pair[this_new_map[0]].append(
                                                                MyMap(source_lang, target_lang, this_new_map[0],
                                                                      this_new_map[1],
                                                                      s_subtrees[0][1], [t_subtree[0][1]],
                                                                      record_anchors))
                                                    else:
                                                        new_path2pair_count += 1
                                                        update_source_path2pair[this_new_map[0]] = [
                                                            MyMap(source_lang, target_lang, this_new_map[0],
                                                                  this_new_map[1],
                                                                  s_subtrees[0][1], [t_subtree[0][1]],
                                                                  record_anchors)]
                                        ...
                                        if pair_item.target_paths == [target_path1]:
                                            ...
                                            if this_new_map[0] in update_source_path2pair:
                                                ...
                                                if not if_contained:
                                                    new_path2pair_count += 1
                                                    update_source_path2pair[this_new_map[0]].append(
                                                        MyMap(source_lang, target_lang, this_new_map[0], this_new_map[1],
                                                              s_subtrees[0][1], [t_subtree[0][1]],
                                                              record_anchors))
                                            else:
                                                new_path2pair_count += 1
        ...
        for root_node, tree_paths in root_node_list.items():
            for source_path1, source_path2 in combinations(tree_paths, 2):
                            ...
                            for target_path2_list in maps[source_path2]:
                                ...
                                if source_path1 in update_source_path2pair:
                                    for pair_item in update_source_path2pair[source_path1]:
                                        if pair_item.target_paths == [target_path1]:
                                            pair_item_anchors = pair_item.anchors
                                            path_s = more_source_diff1[0]
                                            path_t = more_trans_diff1[0]
                                            record_anchors = []
                                            for pair_item_anchor in pair_item_anchors:
                                                s_pair_item_anchor = pair_item_anchor[0]
                                                t_pair_item_anchor = pair_item_anchor[1][0][0]
                                ...
                                if source_path2 in update_source_path2pair:
                                    for pair_item in update_source_path2pair[source_path2]:
                                        if pair_item.target_paths == [target_path2]:
                                            pair_item_anchors = pair_item.anchors
                                            path_s = more_source_diff1[0]
                                            path_t = more_trans_diff1[0]
                                            record_anchors = []
                ...
                for num, s_node_types in s_len2nodetype.items():
                    ...
                    if len(s_node_types) == 1 and len(t_node_types) == 1:
                        ...
                        for s_subtree, t_subtree in zip(s_subtrees, t_subtrees):
                            ...
                            source_path1 = pair.source_path
                            target_path1 = pair.target_paths[0]
                            source_diff_path_type = [s_subtree[0]]
                            target_diff_path_type = [t_subtree[0][0]]
                            if source_path1 in update_source_path2pair:
                                for pair_item in update_source_path2pair[source_path1]:
                                    if pair_item.target_paths == [target_path1]:
