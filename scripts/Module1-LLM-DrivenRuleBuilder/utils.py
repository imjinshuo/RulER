...
def node_type_transfer_for_template(mytree, ori_walk_path, FL_source_tree, template_anchors, source_lang, target_lang, var_dict, var_list, number_dict, type_dict, if_int_same, trans_use_variables):
    node_types = node_type_transfer(target_lang, mytree.type, mytree.text, mytree.variable_names)
    this_type = node_types[0]
    if this_type in ['sized_type_specifier-0', 'primitive_type-0', 'primitive_type-1', 'primitive_type-2', 'primitive_type-3', 'primitive_type-4', 'primitive_type-6', 'primitive_type-7', 'integral_type-0', 'floating_point_type-0', 'long-0']:
        ...
        for anchor in template_anchors:
            ...
            for this_anchor in anchor[1]:
                this_target_anchor = this_anchor[0]
                if ori_walk_path == this_target_anchor:
                    try:
                        if source_anchor:
                            pre_source_subtree = FL_source_tree.getChild(source_anchor[:-1])
                            if pre_source_subtree.type in ['binary_expression', 'binary_operator', 'comparison_operator'] and len(pre_source_subtree.children) == 3 and pre_source_subtree.children[0].type == pre_source_subtree.children[2].type:
                                continue
                        source_subtree = FL_source_tree.getChild(source_anchor)
                    except:
                        continue
                    if source_subtree.children == []:
                        this_type = source_subtree.text
                        var_list.append(source_subtree.text)
                        if this_type != '_':
                            if_anchor = True
                            break
        if not if_anchor:
            if_anchor_loose = False
            for anchor in template_anchors:
                source_anchor = anchor[0]
                target_anchor = anchor[1][0][0]
                if len(ori_walk_path) > len(target_anchor) and ori_walk_path[:len(target_anchor)] == target_anchor:
                    try:
                        source_subtree = FL_source_tree.getChild(source_anchor)
                    except:
                        continue
                    use_variables = []
                    find_use_variable(source_subtree, use_variables, source_lang)
                    if len(use_variables) == 1:
                        this_type = use_variables[0]
                        if_anchor_loose = True
            if not if_anchor_loose:
                var_dict.append(mytree.text)
                var_list.append(mytree.text)
                this_type = '<variable_' + str(len(var_dict)) +'>'
        if mytree.text in ['accumulate', 'static_cast', 'npos']:
        ...
        if if_int_same:
            for anchor in template_anchors:
                source_anchor = anchor[0]
                target_anchor = anchor[1][0][0]
                if ori_walk_path == target_anchor:
                    try:
                        source_subtree = FL_source_tree.getChild(source_anchor)
                    except:
                        continue
                ...
                for anchor in template_anchors:
                    ...
                    if len(ori_walk_path) > len(target_anchor) and ori_walk_path[:len(target_anchor)] == target_anchor:
                        try:
                            ...
                            continue
                        use_variables = []
                        find_use_consts(source_subtree, use_variables)
                        if len(use_variables) == 1:
                            this_type = use_variables[0]
                            if_anchor_loose = True
                if not if_anchor_loose:
        ...
        this_type = mytree.text
    return this_type, var_dict, var_list, number_dict, type_dict
...
def mytree2template(mytree, ori_walk_path, FL_source_tree, template_anchors, source_lang, target_lang, code_str, var_dict, var_list, number_dict, string_dict, type_dict, pretoken, if_int_same, trans_use_variables):
    if 'comment' in mytree.type:
        None
    if mytree.type in ['binary_expression'] and len(mytree.children) == 3 and mytree.children[0].children == [] and mytree.children[2].children == [] and mytree.children[1].type not in ['==', '>', '<', '>=', '<=']:
        node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(MyTree('(', [], False, '(', None, []), None,
                                                                                      FL_source_tree, template_anchors,
                                                                                      source_lang, target_lang,
                                                                                      var_dict, var_list, number_dict, type_dict,
                                                                                      if_int_same, trans_use_variables)
        code_str, pretoken = template_add_str(code_str, node_type, pretoken=pretoken)
        this_ori_walk_path = ori_walk_path[:]
        this_ori_walk_path.append(0)
        if 'e' in mytree.children[0].text and is_number(mytree.children[0].text):
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree.children[0],
                                                                                          this_ori_walk_path, FL_source_tree,
                                                                                          template_anchors, source_lang,
                                                                                          target_lang, var_dict, var_list,
                                                                                          number_dict, type_dict,
                                                                                          if_int_same,
                                                                                          trans_use_variables)
            code_str, pretoken = template_add_str(code_str, node_type, pretoken=pretoken)
            code_str = code_str + 'e'
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree.children[0],
                                                                                          ...
                                                                                          if_int_same,
                                                                                          trans_use_variables)
            code_str, pretoken = template_add_str(code_str, node_type, if_start=True, pretoken=pretoken)
        else:
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree.children[0],
                                                                                          this_ori_walk_path,
                                                                                          FL_source_tree,
                                                                                          template_anchors,
                                                                                          source_lang, target_lang,
                                                                                          var_dict, var_list, number_dict,
                                                                                          type_dict,
                                                                                          if_int_same,
                                                                                          trans_use_variables)
            code_str, pretoken = template_add_str(code_str, node_type, pretoken=pretoken)
        this_ori_walk_path = ori_walk_path[:]
        this_ori_walk_path.append(1)
        node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree.children[1],
                                                                                      this_ori_walk_path,
                                                                                      FL_source_tree,
                                                                                      template_anchors,
                                                                                      source_lang, target_lang,
        ...
        code_str, pretoken = template_add_str(code_str, node_type, pretoken=pretoken)
        this_ori_walk_path = ori_walk_path[:]
        this_ori_walk_path.append(2)
        if 'e' in mytree.children[2].text and is_number(mytree.children[2].text):
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree.children[2],
                                                                                          this_ori_walk_path, FL_source_tree,
                                                                                          template_anchors, source_lang,
                                                                                          target_lang, var_dict, var_list,
                                                                                          number_dict, type_dict,
        ...
        if 'e' in mytree.text and is_number(mytree.text):
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree, ori_walk_path, FL_source_tree, template_anchors, source_lang, target_lang, var_dict, var_list, number_dict, type_dict, if_int_same, trans_use_variables)
            code_str, pretoken = template_add_str(code_str, node_type, pretoken=pretoken)
            code_str = code_str + 'e'
            node_type, var_dict, var_list, number_dict, type_dict = node_type_transfer_for_template(mytree, ori_walk_path, FL_source_tree, template_anchors, source_lang, target_lang, var_dict, var_list, number_dict, type_dict, if_int_same, trans_use_variables)
            code_str, pretoken = template_add_str(code_str, node_type, if_start=True, pretoken=pretoken)
        ...
        for anchor in template_anchors:
            ...
            if ori_walk_path == target_anchor:
                try:
                    ...
                    continue
                if source_subtree.children == []:
                    this_string_text = source_subtree.text
                    if_anchor = True
        if not if_anchor:
            if_anchor_loose = False
            for anchor in template_anchors:
                ...
                if len(ori_walk_path) > len(target_anchor) and ori_walk_path[:len(target_anchor)] == target_anchor:
                    try:
                        ...
                        continue
                    use_variables = []
                    find_use_strings(source_subtree, use_variables)
                    if len(use_variables) == 1:
                        this_string_text = use_variables[0]
                        if_anchor_loose = True
            if not if_anchor_loose:
        ...
        for child_id, child in enumerate(mytree.children):
            if child.type not in ['block', 'compound_statement', 'else', 'elif_clause', 'else_clause', 'catch_clause', 'except_clause']:
                this_ori_walk_path = ori_walk_path[:]
                this_ori_walk_path.append(child_id)
                code_str, var_dict, var_list, number_dict, string_dict, type_dict, pretoken = mytree2template(child, this_ori_walk_path, FL_source_tree, template_anchors, source_lang, target_lang, code_str, var_dict, var_list, number_dict, string_dict, type_dict, pretoken, if_int_same, trans_use_variables)
    return code_str, var_dict, var_list, number_dict, string_dict, type_dict, pretoken
...
def update_var(stmt, stmt_var, stmt_type, use_variables, use_consts, use_strings, use_types, source_lang):
    if '1' not in use_consts:
        use_consts.append('1')
    if 'double' not in use_types:
        use_types.append('double')
    if 'long long' not in use_types:
        use_types.append('long long')
    if 'mpz_class' not in use_types and source_lang == 'Python':
        use_types.append('mpz_class')
    ...
    if stmt_type in ['declaration']:
        if '0' not in use_consts and ('int' in use_types or 'long' in use_types or 'long long' in use_types):
            use_consts.append('0')
...
def check_same(item1, item2):
    ...
    while ' ' in item1:
        item1 = item1.replace(' ', '')
    while 'and' in item1:
        item1 = item1.replace('and', '')
    while 'or' in item1:
        item1 = item1.replace('or', '')
    while 'not' in item1:
        item1 = item1.replace('not', '')
...
def fill_template(stmt, var_dict, number_dict, string_dict, type_dict, stmt_type, if_int_same, ori_use_variables, ori_use_consts, ori_use_strings, ori_use_types,
    ...
    same_consts = []
    if len(source_use_consts) == len(trans_use_consts) and if_int_same:
    ...
    same_strings = []
    if len(source_use_strings) == len(trans_use_strings):
        for var1, var2 in zip(source_use_strings, trans_use_strings):
            if var1 == var2:
                same_strings.append(var1)
            else:
                same_strings.append(None)
    ...
    var_choices = []
    use_variables = [item for item in use_variables if item != 'static_cast']
    ...
    if stmt.startswith('for') and stmt.count(';') == 2 and var_choices and len(var_choices[0]) == 3:
        var_choices = [var_choice for var_choice in var_choices if len(set(var_choice)) == 1]
    const_choices = []
    ...
    new_type_choices = []
    if 'long long' in ori_use_types or 'mpz_class' in ori_use_types:
        new_type_choices = type_choices[:]
    else:
        for type_choice in type_choices:
            if 'long long' in type_choice and 'mpz_class' in type_choice:
                continue
            new_type_choices.append(type_choice)
    if '>e<constant' in stmt:
        new_type_choices = [new_type_choice for new_type_choice in new_type_choices if 'mpz_class' not in new_type_choice and 'double' not in new_type_choice]
    ...
    for type_choice in new_type_choices:
        if type_choice and 'long long' in type_choice and type_choice[0] != 'long long':
            continue
        if type_choice and 'mpz_class' in type_choice and type_choice[0] != 'mpz_class':
            continue
    ...
    new_string_choices = string_choices[:]
    ...
    stmts2 = []
    if number_dict:
        for choice in new_const_choices:
            for stmt in stmts1:
    ...
    stmts3 = []
    if string_dict:
        for choice in new_string_choices:
            for stmt in stmts2:
    ...
    if len(stmts4) > average_len:
        ...
        for stmt in stmts4:
            score = 0
            score += stmt.count('long long') * 2
            score += stmt.count('mpz_class') * 2
            score += stmt.count('{0}')
            score += stmt.count(', 0)')
            score += stmt.count('=')
...
def change_format4end(this_template, var_dict):
    another_template = copy.deepcopy(this_template)
    for id in range(10):
        if f'<variable_{id}>.end()' in another_template:
            another_template = another_template.replace(f'<variable_{id}>.end()', 'std::string::npos')
    for var in var_dict:
        if f'{var}.end()' in another_template:
            another_template = another_template.replace(f'{var}.end()', 'std::string::npos')
...
def compare_stepbystep(source_traces, trans_traces, source_lang, target_lang, line_M, len_s, len_t, source_lines, trans_lines, trans_line_def_variables, fixed_lines):
    report_id = 0
    diff_info = []
    pre_s_state = [[], [], []]
    pre_t_state = [[], [], []]
            ...
            if report_id in fixed_lines:
                ...
                if report_id == 0 and t_state[0] == [0] and if_have_diff_value:
                    ...
                    if not if_find_not_fix_line:
                        report_id = -1
    return report_id, [diff_info, pre_s_state, pre_t_state, s_state, t_state]
...
def validate_buildmap(source_path1, target_path1, map, source_lang, target_lang):
    if '\n-0' in map[0] or '\n-0' in map[1][0]:
        return False
    ...
    if map[1][0] == 'identifier-0' and map[0] != 'identifier-0':
        return False
    if (source_lang == 'Java' and target_lang == 'C++') or (source_lang == 'C++' and target_lang == 'Java'):
        if map[0] == '>' and ''.join(map[1]) == '<':
            return False
        if map[0] == '<' and ''.join(map[1]) == '>':
            return False
        if map[0] == '>=' and ''.join(map[1]) == '<=':
        ...
        if map[0] == '<=' and ''.join(map[1]) == '>=':
            return False
        if map[0] == 'true-0' and ''.join(map[1]) != 'true-0':
            return False
        if ''.join(map[1]) == 'true-0' and map[0] != 'true-0':
            return False
        if map[0] == 'false-0' and ''.join(map[1]) != 'false-0':
            return False
        if ''.join(map[1]) == 'false-0' and map[0] != 'false-0':
            return False
        if map[0].startswith('array_access-0||||identifier-0||||[-0') and not ''.join(map[1]).startswith('subscript_expression-0||||identifier-0||||[-0'):
            return False
        if map[0].startswith('subscript_expression-0||||identifier-0||||[-0') and not ''.join(map[1]).startswith('array_access-0||||identifier-0||||[-0'):
            return False
        if ''.join(map[1]).startswith('array_access-0||||identifier-0||||[-0') and not map[0].startswith('subscript_expression-0||||identifier-0||||[-0'):
            return False
        if ''.join(map[1]).startswith('subscript_expression-0||||identifier-0||||[-0') and not map[0].startswith('array_access-0||||identifier-0||||[-0'):
            return False
        if map[0].startswith('array_access-0||||array_access-0||||identifier-0||||[-0') and not ''.join(map[1]).startswith('subscript_expression-0||||subscript_expression-0||||identifier-0||||[-0'):
            return False
        if map[0].startswith('subscript_expression-0||||subscript_expression-0||||identifier-0||||[-0') and not ''.join(map[1]).startswith('array_access-0||||array_access-0||||identifier-0||||[-0'):
            return False
        if ''.join(map[1]).startswith('array_access-0||||array_access-0||||identifier-0||||[-0') and not map[0].startswith('subscript_expression-0||||subscript_expression-0||||identifier-0||||[-0'):
            return False
        if ''.join(map[1]).startswith('subscript_expression-0||||subscript_expression-0||||identifier-0||||[-0') and not map[0].startswith('array_access-0||||array_access-0||||identifier-0||||[-0'):
            return False
        if map[0].startswith('binary_expression-0||||') and not ''.join(map[1]).startswith('binary_expression-0||||'):
            return False
        if ''.join(map[1]).startswith('binary_expression-0||||') and not map[0].startswith('binary_expression-0||||'):
            return False
    ...
    if source_lang == 'Python' and target_lang == 'C++':
        if map[0].startswith('if_') and ' in ' not in map[0]:
    ...
    if source_lang == 'C++' and target_lang == 'Python':
        if map[0].startswith('if_') and ' in ' not in map[1][0]:
            if map[0].count('==') != map[1][0].count('=='):
                return False
            if map[0].count('>=') != map[1][0].count('>='):
                return False
...
def validate_map(map, source_lang, target_lang):
    ...
    if map[0].startswith('if_statement') and ''.join(map[1]).startswith('parenthesized_expression'):
        return False
    if (source_lang == 'Python' and target_lang == 'C++') or (source_lang == 'C++' and target_lang == 'Python'):
        if map[0].startswith('if_') and (' in ' not in map[0] and ' in ' not in map[1][0]):
            if map[0].count('==') != map[1][0].count('=='):
                return False
            if map[0].count('>=') != map[1][0].count('>='):
                return False
        ...
        if 'comment' in map[0] and 'declaration' in ''.join(map[1]):
            return False
        if 'comment' in map[0] and 'expression_statement' in ''.join(map[1]):
            return False
        if 'comment' in map[0] and 'return_statement' in ''.join(map[1]):
            return False
        ...
        if map[0] == 'break_statement-0||||break-0' and ''.join(map[1]) != 'break_statement-0||||break-0||||;-0':
            return False
        if ''.join(map[1]) == 'break_statement-0||||break-0' and map[0] != 'break_statement-0||||break-0||||;-0':
            return False
        ...
        if map[0].startswith('local_variable_declaration-0||||type_identifier-213||||variable_declarator-0||||'):
            return False
        if map[1][0].startswith('local_variable_declaration-0||||type_identifier-213||||variable_declarator-0||||'):
            return False
        if map[0].startswith('return_statement-0||||') and not map[1][0].startswith('return_statement-0||||'):
            return False
        if map[1][0].startswith('return_statement-0||||') and not map[0].startswith('return_statement-0||||'):
        ...
        if map[0] == 'break_statement-0||||break-0||||;-0' and ''.join(map[1]) != 'break_statement-0||||break-0||||;-0':
            return False
        if ''.join(map[1]) == 'break_statement-0||||break-0||||;-0' and map[0] != 'break_statement-0||||break-0||||;-0':
            return False
        if map[0] == 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||true-0||||;-0' and ''.join(map[1]) != 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||true-0||||;-0':
            return False
        if ''.join(map[1]) == 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||true-0||||;-0' and map[0] != 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||true-0||||;-0':
            return False
        if map[0] == 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||false-0||||;-0' and ''.join(map[1]) != 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||false-0||||;-0':
            return False
        if ''.join(map[1]) == 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||false-0||||;-0' and map[0] != 'expression_statement-0||||assignment_expression-0||||identifier-0||||=-0||||false-0||||;-0':
            return False

    if source_lang == 'Python' and target_lang == 'C++':
        if 'identifier-66' in map[0] and 'identifier-70' not in ''.join(map[1]):
            return False
        if 'identifier-70' in ''.join(map[1]) and 'identifier-66' not in map[0]:
            return False
        if 'identifier-76' in map[0] and 'identifier-306' not in ''.join(map[1]):
            return False
        if 'identifier-306' in ''.join(map[1]) and 'identifier-76' not in map[0]:
            return False
        if map[0].startswith('for_statement-0||||') and 'expression_statement' in ''.join(map[1]) and 'identifier-70' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('if_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('for_statement-0||||') and ''.join(map[1]).count('declaration-0') > 1 and ('identifier-70' not in ''.join(map[1]) and 'identifier-134' not in map[0]):
            return False
        if map[0].startswith('while_statement-0||||') and ''.join(map[1]).count('declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).count('declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('and') != ''.join(map[1]).count('&&'):
            return False
        if 'break_statement-0||||break-0' in map[0] and 'break_statement-0||||break-0||||;-0' not in ''.join(map[1]):
            return False
        if 'break_statement-0||||break-0||||;-0' in ''.join(map[1]) and 'break_statement-0||||break-0' not in map[0]:
            return False
        if 'expression_statement-0||||call_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-0||||argument_list-0||||(-0||||false-0||||)-0||||;-0' in ''.join(map[1]):
            return False
        if 'expression_statement-0||||call_expression-0||||field_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-70||||.-0||||field_identifier-391||||argument_list-0||||(-0||||nullptr-0||||)-0||||;-0' in ''.join(map[1]):
            return False

    if source_lang == 'C++' and target_lang == 'Python':
        if 'identifier-70' in map[0] and 'identifier-66' not in ''.join(map[1]):
            return False
        if 'identifier-66' in ''.join(map[1]) and 'identifier-70' not in map[0]:
            return False
        if 'identifier-306' in map[0] and 'identifier-76' not in ''.join(map[1]):
            return False
        if 'identifier-76' in ''.join(map[1]) and 'identifier-306' not in map[0]:
            return False
        if map[0].startswith('for_statement-0||||') and 'expression_statement' in ''.join(map[1]) and 'identifier-66' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('if_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('for_statement-0||||') and ''.join(map[1]).count('declaration-0') > 1 and 'identifier-66' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and ''.join(map[1]).count('declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).count('declaration-0'):
        ...
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('and') != ''.join(map[1]).count('&&'):
            return False
        if 'break_statement-0||||break-0||||;-0' in map[0] and 'break_statement-0||||break-0' not in ''.join(map[1]):
            return False
        if 'break_statement-0||||break-0' in ''.join(map[1]) and 'break_statement-0||||break-0||||;-0' not in map[0]:
            return False

    if source_lang == 'Java' and target_lang == 'C++':
        if 'identifier-1389' in map[0] and 'identifier-70' not in ''.join(map[1]):
            return False
        if 'identifier-70' in ''.join(map[1]) and 'identifier-1389' not in map[0]:
            return False
        if 'identifier-769' in map[0] and 'identifier-306' not in ''.join(map[1]):
            return False
        if 'identifier-306' in ''.join(map[1]) and 'identifier-769' not in map[0]:
            return False
        if map[0].startswith('for_statement-0||||') and 'expression_statement' in ''.join(map[1]) and 'identifier-70' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and 'expression_statement' in ''.join(map[1]):
        ...
        if map[0].startswith('if_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('for_statement-0||||') and ''.join(map[1]).count('declaration-0') > 1 and 'identifier-70' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and ''.join(map[1]).count('declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).count('declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('&&') != ''.join(map[1]).count('&&'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('||') != ''.join(map[1]).count('||'):
            return False
        if 'break_statement-0||||break-0||||;-0' in map[0] and 'break_statement-0||||break-0||||;-0' not in ''.join(map[1]):
            return False
        if 'break_statement-0||||break-0||||;-0' in ''.join(map[1]) and 'break_statement-0||||break-0||||;-0' not in map[0]:
            return False
        if 'expression_statement-0||||call_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-0||||argument_list-0||||(-0||||false-0||||)-0||||;-0' in ''.join(map[1]):
            return False
        if 'expression_statement-0||||call_expression-0||||field_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-70||||.-0||||field_identifier-391||||argument_list-0||||(-0||||nullptr-0||||)-0||||;-0' in ''.join(map[1]):
            return False
        if 'local_variable_declaration-0||||integral_type-0||||' in map[0] and '||||variable_declarator-0||||identifier-0||||;-0' in map[0] and ''.join(map[1]) != 'declaration-0||||primitive_type-0||||identifier-0||||;-0':
            return False

    if source_lang == 'C++' and target_lang == 'Java':
        if 'identifier-70' in map[0] and 'identifier-1389' not in ''.join(map[1]):
            return False
        if 'identifier-1389' in ''.join(map[1]) and 'identifier-70' not in map[0]:
            return False
        if 'identifier-306' in map[0] and 'identifier-769' not in ''.join(map[1]):
            return False
        if 'identifier-769' in ''.join(map[1]) and 'identifier-306' not in map[0]:
            return False
        if map[0].startswith('for_statement-0||||') and 'expression_statement' in ''.join(map[1]) and 'identifier-1389' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('if_statement-0||||') and 'expression_statement' in ''.join(map[1]):
            return False
        if map[0].startswith('for_statement-0||||') and ''.join(map[1]).count('local_variable_declaration-0') > 1 and 'identifier-1389' not in ''.join(map[1]):
            return False
        if map[0].startswith('while_statement-0||||') and ''.join(map[1]).count('local_variable_declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).count('local_variable_declaration-0'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('&&') != ''.join(map[1]).count('&&'):
            return False
        if map[0].startswith('if_statement-0||||') and ''.join(map[1]).startswith('if_statement-0||||') and map[0].count('||') != ''.join(map[1]).count('||'):
        ...
        if 'break_statement-0||||break-0||||;-0' in ''.join(map[1]) and 'break_statement-0||||break-0||||;-0' not in map[0]:
            return False
        if 'expression_statement-0||||call_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-0||||argument_list-0||||(-0||||false-0||||)-0||||;-0' in map[0]:
            return False
        if 'expression_statement-0||||call_expression-0||||field_expression-0||||qualified_identifier-0||||namespace_identifier-0||||::-0||||identifier-70||||.-0||||field_identifier-391||||argument_list-0||||(-0||||nullptr-0||||)-0||||;-0' in map[0]:
            return False
        if 'local_variable_declaration-0||||integral_type-0||||' in ''.join(map[1]) and '||||variable_declarator-0||||identifier-0||||;-0' in ''.join(map[1]) and map[0] != 'declaration-0||||primitive_type-0||||identifier-0||||;-0':
            return False
    return True


def validate_sim_map(map, source_lang, target_lang):
    if 'comment' in map[0] and 'comment' not in ''.join(map[1]):
        return False
    if 'comment' not in map[0] and 'comment' in ''.join(map[1]):
        return False
...
def read_code(file_path, lang):
    ...
    if lang == 'Java':
        ...
        for line_id, line in enumerate(lines):
            ...
            filter_lines.append(this_line)
        pre_new_lines = []
        new_lines =filter_lines[:]
        while pre_new_lines != new_lines:
            pre_new_lines = new_lines[:]
            for line_id, line in enumerate(new_lines):
                ...
                for i in range(index_len):
                    index_str += ' '
                if '-- ) for (' in line:
                    if line_id+3 < len(new_lines):
                        if new_lines[line_id+2].endswith('{\n'):
                            if_added = False
                            for this_line_id in range(line_id+3, len(new_lines)):
                                if new_lines[this_line_id] == index_str+'}\n':
                                    new_lines[this_line_id] = new_lines[this_line_id][:-1] + '}\n'
                                    if_added = True
                                    break
                            if if_added:
                                new_lines[line_id] = new_lines[line_id].replace('-- ) for (', '-- ) { for (')
                        else:
                            new_lines[line_id] = new_lines[line_id].replace('-- ) for (', '-- ) { for (')
                            new_lines[line_id + 2] = new_lines[line_id+2][:-1] + '}\n'
                        break
                    else:
                        continue
                elif '++ ) for (' in line:
                        ...
                        if new_lines[line_id+2].endswith('{\n'):
                            ...
                            for this_line_id in range(line_id+3, len(new_lines)):
                                if new_lines[this_line_id] == index_str+'}\n':
                                    ...
                                    break
                            if if_added:
                                new_lines[line_id] = new_lines[line_id].replace('1 ) for (', '1 ) { for (')
                        else:
                            new_lines[line_id] = new_lines[line_id].replace('1 ) for (', '1 ) { for (')
                            new_lines[line_id + 2] = new_lines[line_id+2][:-1] + '}\n'
                        break
...
def match(ori_tree, ori_walk_path, source_lang, target_lang, maps, trans_path2tree, path_path2anchors):
    ...
    if ori_subtree_path in maps:
        ...
        for this_target_tree_paths in maps[ori_subtree_path]:
            ...
            this_target_anchors = []
            this_pathpath = ori_subtree_path + '>>>>' + '####'.join([this_target_tree_path for this_target_tree_path in this_target_tree_paths if this_target_tree_path in trans_path2tree])
            if this_pathpath in path_path2anchors:
                this_target_anchors.extend(path_path2anchors[this_pathpath])
            if_contained = False
            for this_possible_subtrees in possible_subtrees:
                if len(this_possible_subtrees) == len(this_target_trees):
                    if_diff = False
                    for this_possible_subtree, this_target_tree in zip(this_possible_subtrees[0], this_target_trees):
                    ...
                    if not if_diff:
                        if_contained = True
            if not if_contained:
                possible_subtrees.append([this_target_trees, this_target_anchors])
...
def get_set(lists_of_choices, source_lang):
    ...
    for list_id, list_of_choices in enumerate(lists_of_choices):
        text = '####'.join(['||||'.join(item.getDFS(source_lang)) for item in list_of_choices[0]])
...
def match_force_begin(ori_tree, ori_walk_path, root_node2map, source_lang, target_lang, maps, source_path2tree, trans_path2tree, depth, max_depth, max_possible_choices, start_time, time_limit, if_search, exis_possible_maps_list, path_path2anchors):
    this_time = time.time()
    ...
    if ori_subtree_type[0] in root_node2map:
        ...
        for choice_id, map_choice in enumerate(map_choices):
            ...
            choice_target_nodes = map_choice.target_paths[0].split('||||')
            BLEUscore = nltk.translate.bleu_score.sentence_bleu([ori_nodes], choice_nodes, weights=(0.5, 0.5))
            ori_map_choices_counts.append([BLEUscore, choice_id])
        ori_map_choices_counts.sort(reverse=True)
        map_choices_counts = []
        for choice_item in ori_map_choices_counts[:20]:
            choice_id = choice_item[1]
            map_choice = map_choices[choice_id]
            if ori_subtree_path == map_choice.source_path:
                continue
            choice_target_nodes = map_choice.target_paths[0].split('||||')
            if exis_possible_maps_list:
                target_BLEUscore = nltk.translate.bleu_score.sentence_bleu(exis_possible_maps_list, choice_target_nodes, weights=(0.5, 0.5))
            else:
                target_BLEUscore = 1.0
            if target_BLEUscore > 0.6:
                continue
            map_choices_counts.append([target_BLEUscore, choice_id])
        map_choices_counts.sort()
        fail_source_paths = []
        succ_infos = {}
        if_enough = 0
        for map_choice_id, map_choice_count in enumerate(map_choices_counts):
            ...
            for this_diff_path in diff_paths:
                this_exact_matched_uncompared_idxs = []
                this_loose_matched_uncompared_idxs = []
                ...
                for uncompared_idx, uncompared_ids in enumerate(map_choice_source_anchors):
                    ...
                    if len(this_diff_path_ids) == len(uncompared_ids):
                        if this_diff_path_ids == uncompared_ids:
                            if_exact_match = True
                            this_exact_matched_uncompared_idxs.append(uncompared_idx)
                    elif len(this_diff_path_ids) > len(uncompared_ids):
                        if this_diff_path_ids[:len(uncompared_ids)] == uncompared_ids:
                            if_loose_match = True
                            if this_loose_matched_uncompared_idxs and len(map_choice_source_anchors[this_loose_matched_uncompared_idxs[0]]) < len(map_choice_source_anchors[uncompared_idx]):
                                this_loose_matched_uncompared_idxs = [uncompared_idx]
                            else:
                                this_loose_matched_uncompared_idxs.append(uncompared_idx)
                if not if_exact_match and not if_loose_match:
                    if_valid = False
                if this_exact_matched_uncompared_idxs:
                    exact_matched_uncompared_idxs.extend(this_exact_matched_uncompared_idxs)
                elif this_loose_matched_uncompared_idxs:
                    loose_matched_uncompared_idxs.extend(this_loose_matched_uncompared_idxs)
            ...
            if if_valid and not if_contain_fail_source_path:
                ...
                base_trans_anchors = copy.deepcopy(map_choice.anchors)
                this_possible_subtrees_list.append([base_trans_trees, base_trans_anchors])
                for matched_uncompared_idx in filter_matched_uncompared_idxs:
                    ...
                    for uncompared_target_anchor in uncompared_target_anchors:
                        ...
                        this_this_possible_anchors_list = []
                        for return_subtree, return_subtree_anchors in return_subtrees:
                            if len(return_subtree) != 1:
                                continue
                            for this_possible_subtrees, this_possible_anchors in this_possible_subtrees_list:
                                try:
                                    this_this_possible_anchors = []
                                    this_this_pre_trans_trees = copy.deepcopy(this_possible_subtrees)
                                    if this_this_pre_trans_trees[uncompared_target_stmt_ids].getChild(uncompared_target_ids).type == 'argument_list':
                                        if return_subtree[0].type != 'argument_list':
                                            continue
                                    ...
                                    for this_possible_anchor in this_possible_anchors:
                                        this_anchor_source_anchor = this_possible_anchor[0]
                                        this_anchor_target_anchor = this_possible_anchor[1][0]
                                        if this_anchor_target_anchor[1] != uncompared_target_stmt_ids:
                                            continue
                                        if len(this_anchor_target_anchor[0]) > len(uncompared_target_ids) and this_anchor_target_anchor[0][:len(uncompared_target_ids)] == uncompared_target_ids:
                                            continue
                                        this_this_possible_anchors.append(this_possible_anchor)
                                    for this_new_anchor in return_subtree_anchors:
                                        this_new_source_anchor = this_new_anchor[0]
                                        this_new_target_anchor = this_new_anchor[1][0]
                                        this_new_new_source_anchor = uncompared_ids[:]
                                        this_new_new_target_anchor = uncompared_target_ids[:]
                                        this_new_new_source_anchor.extend(this_new_source_anchor)
                                        this_new_new_target_anchor.extend(this_new_target_anchor[0])
                                        this_this_possible_anchors.append([this_new_new_source_anchor, [[this_new_new_target_anchor, 0]]])
                                    this_this_possible_anchors_list.append(this_this_possible_anchors)
                                except:
                                    continue

                        this_possible_subtrees_list = [[item1, item2] for item1, item2 in zip(this_this_possible_subtrees_list, this_this_possible_anchors_list)]
...
def match_force(ori_tree, ori_walk_path, root_node2map, source_lang, target_lang, maps, source_path2tree, trans_path2tree, depth, max_depth, max_possible_choices, start_time, time_limit, if_search, path_path2anchors):
    this_time = time.time()
    if this_time - start_time > time_limit:
        print('Time out...')
        return [], if_search
    ...
    ori_subtree_children = ori_subtree.children
    ori_subtree_type = node_type_transfer(source_lang, ori_subtree.type, ori_subtree.text, ori_subtree.variable_names)
    if (ori_subtree_path not in maps and ori_subtree_path.startswith('identifier-') and '||||' not in ori_subtree_path) or (ori_subtree_type[0] not in root_node2map and ori_subtree_type[0].startswith('identifier-')):
        ori_subtree_path = 'identifier-0'
        ori_subtree_type[0] = 'identifier-0'
    if depth >= max_depth:
        return [], if_search
    if ori_subtree_path in maps:
        if_search = True
        possible_subtrees = []
        for this_target_tree_paths_id, this_target_tree_paths in enumerate(maps[ori_subtree_path]):
            this_target_trees = [trans_path2tree[this_target_tree_path] for this_target_tree_path in
                                 this_target_tree_paths if this_target_tree_path in trans_path2tree]
            this_target_anchors = []
            this_pathpath = ori_subtree_path + '>>>>' + '####'.join([this_target_tree_path for this_target_tree_path in this_target_tree_paths if this_target_tree_path in trans_path2tree])
            ...
            if not if_contained:
                possible_subtrees.append([this_target_trees, this_target_anchors])
        possible_subtrees = get_set(possible_subtrees, source_lang)
        if ori_subtree_path == 'unary_operator-0||||--0||||integer-0':
            possible_subtrees = [possible_subtree for possible_subtree in possible_subtrees if possible_subtree[0][0].type == 'number_literal']
        return possible_subtrees, if_search
...
def is_prefix_sublist(a, b):
    if len(b) > len(a):
        return False
    return a[:len(b)] == b


def rephrase_maps_for_tree(possible_maps_list, lang):
    if lang == 'C++':
        new_possible_maps_list = []
        for possible_map in possible_maps_list:
            if len(possible_map[0]) == 1 and possible_map[0][0].type == 'if_statement' and possible_map[0][0].children and possible_map[0][0].children[1].type != 'condition_clause':
                possible_map_tree = possible_map[0][0]
                binary_expression_child = copy.deepcopy(possible_map_tree.children[1])
                child_mytree_1 = MyTree('(', [], False, '(', possible_map_tree.line_id, possible_map_tree.variable_names)
                child_mytree_2 = MyTree(')', [], False, ')', possible_map_tree.line_id, possible_map_tree.variable_names)
                mytree_3 = MyTree('condition_clause', [child_mytree_1, binary_expression_child, child_mytree_2], True, f'({binary_expression_child.text})', possible_map_tree.line_id, possible_map_tree.variable_names)
                new_possible_map_tree = copy.deepcopy(possible_map_tree)
                new_possible_map_tree.changeChild([1], mytree_3)
                new_possible_maps_list.append([[new_possible_map_tree]])
            else:
                new_possible_maps_list.append(possible_map)
        return new_possible_maps_list
    else:
        return possible_maps_list


def rephrase_maps(possible_maps_list, lang):
    if lang == 'C++':
        new_possible_maps_list = []
        for possible_map in possible_maps_list:
            if len(possible_map[0]) == 1 and possible_map[0][0].type == 'if_statement' and possible_map[0][0].children and possible_map[0][0].children[1].type != 'condition_clause':
                possible_map_tree = possible_map[0][0]
                possible_map_anchors = possible_map[1]
                binary_expression_child = copy.deepcopy(possible_map_tree.children[1])
                child_mytree_1 = MyTree('(', [], False, '(', possible_map_tree.line_id, possible_map_tree.variable_names)
                child_mytree_2 = MyTree(')', [], False, ')', possible_map_tree.line_id, possible_map_tree.variable_names)
                mytree_3 = MyTree('condition_clause', [child_mytree_1, binary_expression_child, child_mytree_2], True, f'({binary_expression_child.text})', possible_map_tree.line_id, possible_map_tree.variable_names)
                new_possible_map_tree = copy.deepcopy(possible_map_tree)
                new_possible_map_tree.changeChild([1], mytree_3)
                new_possible_map_anchors = [[[1], [[[1], 0]]]]
                for possible_map_anchor in possible_map_anchors:
                    if is_prefix_sublist(possible_map_anchor[0], [1]):
                        new_s_anchor = possible_map_anchor[0][:]
                        new_s_anchor.insert(0, 1)
                        new_t_anchors = []
                        for t_anchor in possible_map_anchor[1]:
                            if is_prefix_sublist(t_anchor[0], [1]) and t_anchor[1] == 0:
                                this_new_t_anchor = t_anchor[0]
                                this_new_t_anchor.insert(0, 1)
                                new_t_anchors.append([this_new_t_anchor, 0])
                            else:
                                new_t_anchors.append(t_anchor)
                        if new_t_anchors:
                            new_possible_map_anchors.append([new_s_anchor, new_t_anchors])
                    else:
                        new_possible_map_anchors.append(possible_map_anchor)
                new_possible_maps_list.append([[new_possible_map_tree], new_possible_map_anchors])
            else:
                new_possible_maps_list.append(possible_map)
        return new_possible_maps_list
    else:
...
def remove_parenthesis_recurve(tree, path, anchors):
    if path:
        target_tree = tree.getChild(path)
    else:
        target_tree = copy.deepcopy(tree)
    if len(target_tree.children) == 3 and target_tree.children[0].text == '(' and target_tree.children[2].text == ')' and len(target_tree.children[1].children) >= 3 and target_tree.children[1].children[0].text == '(' and target_tree.children[1].children[-1].text == ')':
        tree.changeChild(path, target_tree.children[1])
        new_anchors = []
        for anchor in anchors:
            new_target_anchors = []
            for target_anchor in anchor[1]:
                target_path = target_anchor[0]
                stmt_id = target_anchor[1]
                if len(target_path) > len(path) and target_path[:len(path)] == path:
                    this_target_path = target_path[:len(path)-1]
                    this_target_path.extend(target_path[len(path):])
                    new_target_anchors.append([this_target_path, stmt_id])
                else:
                    new_target_anchors.append([target_path, stmt_id])
            if [anchor[0], new_target_anchors] not in new_anchors:
                new_anchors.append([anchor[0], new_target_anchors])
        anchors = new_anchors[:]
    else:
        for child_id, child in enumerate(target_tree.children):
            this_path = path[:]
            this_path.append(child_id)
            anchors = remove_parenthesis_recurve(tree, this_path, anchors)
    return anchors


def remove_parenthesis(trees):
    new_trees = []
    for tree_info in trees:
        if len(tree_info[0]) == 1:
            tree = tree_info[0][0]
            anchors = tree_info[1]
            new_anchors = remove_parenthesis_recurve(tree, [], anchors)
            new_trees.append([[copy.deepcopy(tree)], new_anchors])
        else:
...
class MyTree(object):
    ...
    def changeChild(self, path, new_child):
        if len(path) == 1:
            ...
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].copy(new_child)
        elif len(path) == 12:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].copy(new_child)
        elif len(path) == 13:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].copy(new_child)
        elif len(path) == 14:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].copy(new_child)
        elif len(path) == 15:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].copy(new_child)
        elif len(path) == 16:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].children[path[15]].copy(new_child)
        elif len(path) == 17:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].children[path[15]].children[path[16]].copy(new_child)
        elif len(path) == 18:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].children[path[15]].children[path[16]].children[path[17]].copy(new_child)
        elif len(path) == 19:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].children[path[15]].children[path[16]].children[path[17]].children[path[18]].copy(new_child)
        elif len(path) == 20:
            self.children[path[0]].children[path[1]].children[path[2]].children[path[3]].children[path[4]].children[path[5]].children[path[6]].children[path[7]].children[path[8]].children[path[9]].children[path[10]].children[path[11]].children[path[12]].children[path[13]].children[path[14]].children[path[15]].children[path[16]].children[path[17]].children[path[18]].children[path[9]].copy(new_child)
        else:
...
def mytree2mytree_wo_std_CPP(mytree):
    new_children = []
    for child in mytree.children:
        if child.type in ['qualified_identifier'] and len(child.children) == 3:
            new_children.append(child.children[2])
        else:
            new_children.append(child)
    mytree.children = new_children[:]
    for child in mytree.children:
        mytree2mytree_wo_std_CPP(child)


def mytree2mytree_wo_call_CPP(mytree, if_for):
    if mytree.type in ['for_statement']:
        if_for = True
    if mytree.type in ['call_expression'] and if_for:
        mytree.type = 'identifier'
        mytree.children = []
    for child in mytree.children:
        mytree2mytree_wo_call_CPP(child, if_for)


def mytree2mytree_wo_sub_CPP(mytree, if_for):
    if mytree.type in ['if_statement', 'for_statement']:
        if_for = True
    if mytree.type in ['subscript_expression'] and if_for:
        mytree.type = 'identifier'
        mytree.children = []
    for child in mytree.children:
        mytree2mytree_wo_sub_CPP(child, if_for)


def mytree2mytree_wo_update_CPP(mytree):
    if mytree.type in ['update_expression'] and len(mytree.children) == 2 and mytree.children[0].type in ['++', '--']:
        my_children = mytree.children[:]
        mytree.children = [copy.deepcopy(my_children[1]), copy.deepcopy(my_children[0])]
    for child in mytree.children:
        mytree2mytree_wo_update_CPP(child)


def mytree2mytree_wo_type_CPP(mytree):
    if mytree.type in ['sized_type_specifier', 'primitive_type']:
        mytree.type = 'type_identifier'
        mytree.children = []
    mytree.children = [child for child in mytree.children if child.type not in ['sized_type_specifier', 'primitive_type']]
    for child in mytree.children:
        mytree2mytree_wo_type_CPP(child)


def mytree2mytree_wo_type_JAVA(mytree):
    if mytree.type in ['integral_type', 'type_identifier', 'floating_point_type']:
        mytree.type = 'type_identifier'
        mytree.children = []
    mytree.children = [child for child in mytree.children if child.type not in ['integral_type', 'type_identifier', 'floating_point_type']]
    for child in mytree.children:
        mytree2mytree_wo_type_JAVA(child)
...
def compare_MyTree(tree1, tree2, lang):
    ...
    diff_paths = []
    for diff_path in diff_MyTree(new_tree1, new_tree2, lang, brother=0, pre_node=None):
        if diff_path:
            diff_paths.append(diff_path)
    return diff_paths


def compare_MyTree_more(tree1, tree2, lang):
    new_tree1 = copy.deepcopy(tree1)
    new_tree2 = copy.deepcopy(tree2)
    if lang == 'C++':
        mytree2mytree_wo_std_CPP(new_tree1)
        mytree2mytree_wo_type_CPP(new_tree1)
        mytree2mytree_wo_update_CPP(new_tree1)
        mytree2mytree_wo_sub_CPP(new_tree1, False)
        mytree2mytree_wo_call_CPP(new_tree1, False)
        mytree2mytree_wo_std_CPP(new_tree2)
        mytree2mytree_wo_type_CPP(new_tree2)
        mytree2mytree_wo_update_CPP(new_tree2)
        mytree2mytree_wo_sub_CPP(new_tree2, False)
        mytree2mytree_wo_call_CPP(new_tree2, False)
    if lang == 'Java':
        mytree2mytree_wo_type_JAVA(new_tree1)
        mytree2mytree_wo_type_JAVA(new_tree2)
    diff_paths = []
    for diff_path in diff_MyTree(new_tree1, new_tree2, lang, brother=0, pre_node=None):
        if diff_path:
            diff_paths.append(diff_path)
...
def traverse_tree_number(node, record):
    for n_id, n in enumerate(node.children):
        if n.type in ['number_literal', 'integer', 'decimal_integer_literal']:
            record.append(n.text)
        traverse_tree_number(n, record)
...
def Python_identifier(node_str):
    contained_Ft = []
            ...
            if n.is_named:
                ...
                if (lang == 'Python' and n.type in python_block_name) or (lang == 'Java' and n.type in java_block_name) or (lang == 'C++' and n.type in cpp_block_name):
                    ...
                    if only_path:
                        ...
                        stmts.append(item2)
                        stmts.append(item3)
                        sub_stmt_list = traverse_tree(n, lang, code_lines, variable_names, only_block=False,
                                                      exclude_last_child=False, only_path=only_path,
                                                      fun_block=fun_block + 3, mine=mine,
                                                      target_stmt_id=[4])
                        stmts.extend(sub_stmt_list)
                    else:
                            ...
                            if (lang == 'Python' and this_n.type in python_block_name) or (lang == 'Java' and this_n.type in java_block_name) or (lang == 'C++' and this_n.type in cpp_block_name):
                                ...
                                for block_child in this_n.children:
                                    if not block_child.is_named:
                                        this_map.extend(calculate_map(block_child.start_point, block_child.end_point, code_lines))
                        sub_stmt_list = traverse_tree(n, lang, code_lines, variable_names, only_block=True, exclude_last_child=False, fun_block=fun_block + 1, mine=mine)
                        this_root_path = copy.deepcopy(sub_stmt_list)
                        this_root_path.append([this_path, this_root_my_tree, this_map, fun_block])
                        stmts.append(this_root_path)
...
def find_variable_for_def(tree, variables, lang, start=True):
    for child in tree.children:
        ...
        if 'Scanner' in text:
            continue
        if child.type in ['assignment']:
            start = True
        if child.type in ['[', '++', '--', '('] or '=' in child.type:
            start = False
        node_types = node_type_transfer(lang, child.type, child.text, [])
        this_type = node_types[0]
        if start and (this_type.startswith('identifier-') or child.text in ['sum', 'count']):
            if type(child) == tree_sitter.Node:
                if child.text.decode() not in ['print', 'main']:
                    variables.append(child.text.decode())
            else:
                if child.text not in ['print', 'main']:
...
def find_variable_for_func(tree, variables, lang, start=False):
    ...
    for child in tree.children:
        ...
        node_types = node_type_transfer(lang, child.type, child.text, [])
        this_type = node_types[0]
        if start and this_type.startswith('identifier-'):
            if type(child) == tree_sitter.Node:
                variables.append(child.text.decode())
            else:
                variables.append(child.text)
...
def verify_build_mapping(tree1, trees2, def_vars1, def_vars2, source_lang, target_lang):
    ...
    this_usesd_variables1 = []
    find_use_variable(tree1, this_usesd_variables1, source_lang)
    this_usesd_variables_filt1 = []
    for var in this_usesd_variables1:
    ...
    if set(this_usesd_variables1) == set(this_usesd_variables2) and set(this_def_variables1) == set(this_usesd_variables1) and set(this_def_variables2) == set():
        return True
    if set(this_usesd_variables1) == set(this_usesd_variables2) and set(this_def_variables2) == set(this_usesd_variables2) and set(this_def_variables1) == set():
        return True
...
def code_parse_for_map(lang, code):
    ...
    if lang == 'C++':
        ...
        variable_names = []
        ori_source_stmt_info_lists = traverse_tree(tree.root_node, lang, code,
                                                    variable_names, only_block=False,
                                                    exclude_last_child=False, only_path=True,
                                                    fun_block=0)
        for ori_source_stmt_info_list in ori_source_stmt_info_lists:
            find_def_variable(ori_source_stmt_info_list[1], variable_names, lang)
        variable_names = list(set(variable_names))
        return tree, variable_names
    if lang == 'Python':
    ...
    if lang == 'Java':
        ...
        variable_names = []
        ori_source_stmt_info_lists = traverse_tree(tree.root_node.children[0].children[3], lang, pre_code,
                                                    variable_names, only_block=False,
                                                    exclude_last_child=False, only_path=True,
                                                    fun_block=0)
        for ori_source_stmt_info_list in ori_source_stmt_info_lists:
...
def delete(lang, code_lines):
    ...
    def this_find_use_variable(tree, variables, lang):
        this_type = tree.type
        if this_type == 'identifier':
            if type(tree) == tree_sitter.Node:
                variables.append(tree.text.decode())
                ...
                variables.append(tree.text)
        for child in tree.children:
            this_find_use_variable(child, variables, lang)
    ...
    if lang == 'Java':
        ...
        stmt_lists = traverse_tree(tree.root_node.children[0], lang, code_lines, variable_names, only_block=False, exclude_last_child=False, fun_block=0)
    if stmt_lists and 'ERROR' in stmt_lists[0]:
        return {}
    if stmt_lists and stmt_lists[0] and 'ERROR' in stmt_lists[0][0]:
        return {}
    stmt_lists[0] = stmt_lists[0][::-1]
    def flatten_top_nested(data):
        res = []
        for elem in data:
            if isinstance(elem, list) and len(elem) == 4 and isinstance(elem[2], list) and isinstance(elem[3], int):
                res.append(elem)
            elif isinstance(elem, list):
                res.extend(flatten_top_nested(elem))
            else:
                res.append(elem)
        return res
    stmt_lists = flatten_top_nested(stmt_lists)
    ...
    stmt_lists.sort(key=sort_key, reverse=True)

    def prefix_slices(lst):
        return [lst[:i] for i in range(len(lst) + 1)]

    stmt_lists = prefix_slices(stmt_lists)

    delete_code_dict = {}
    delete_code_list = []
    path2tree = {}
    for stmt_list_id, stmt_list in enumerate(stmt_lists):
        this_deleted_code_lines = copy.deepcopy(code_lines)
        for item_id, item in enumerate(stmt_list):
            this_deleted_code_lines = delete_code(this_deleted_code_lines, item[2])
            if item[0] not in path2tree:
                path2tree[item[0]] = [item[1]]
        if stmt_list != []:
            stmt_list_lst_item_depth = stmt_list[-1][3]
            left_stmts = stmt_lists[-1][len(stmt_list):]
            def_variables = []
            for stmt_id, stmt in enumerate(left_stmts):
                if stmt_id == 0 and stmt[3] <= stmt_list_lst_item_depth:
                    stmt_tree = stmt[1]
                    find_def_variable(stmt_tree, def_variables, lang)
                elif stmt_id != 0 and stmt[3] == 1:
                    stmt_str = stmt[0]
                    if stmt_id!=0 and stmt_str.startswith('for_'):
                        continue
                    stmt_tree = stmt[1]
                    find_def_variable(stmt_tree, def_variables, lang)
            def_variables = set(def_variables)
            if lang in ['Java', 'C++']:
                def_variables_filter = []
                for def_variable in def_variables:
                    if_init = False
                    for this_deleted_code_line in this_deleted_code_lines:
                        if def_variable in this_deleted_code_line and '=' in this_deleted_code_line:
                            if_init = True
                    if if_init:
                        def_variables_filter.append(def_variable)
                def_variables = set(def_variables_filter)
            if def_variables:
                last_pos = stmt_list[-1][2][0]
                if lang == 'Python':
                    def_variables_str = ', "||||", '.join(def_variables)
                    this_deleted_code_lines[last_pos[0]] = this_deleted_code_lines[last_pos[0]][:last_pos[1]] + f'print(\"RulEROUTPUT:\", {def_variables_str})' + this_deleted_code_lines[last_pos[0]][last_pos[1]:]
                elif lang == 'Java':
                    def_variables_str = ', '.join(def_variables)
                    this_deleted_code_lines[last_pos[0]] = this_deleted_code_lines[last_pos[0]][:last_pos[1]] + f'print(\"RulEROUTPUT:\", {def_variables_str});' + this_deleted_code_lines[last_pos[0]][last_pos[1]:]
                elif lang == 'C++':
            ...
            delete_code_list.append([this_deleted_code_lines, [[this_stmt[0], this_stmt[2], this_stmt[3]] for this_stmt in stmt_list]])
    for idx, i in enumerate(delete_code_list[:]):
        if idx == 0:
            continue
        j = delete_code_list[idx-1]
        code1 = i[0]
        code2 = j[0]
        stmt1 = i[1]
        stmt2 = j[1]
...
def update_map(line_M, source_lines, trans_lines):
    ...
    for closure in closures:
        ...
        for this_s in S:
            for this_t in F:
                line_M[f'{this_s}-{this_t}'] = True
    return line_M


def verify_map_vars(s_id, mapped_trans_stmts, source_stmt_def_variables, trans_stmt_def_variables, source_stmt_use_variables, trans_stmt_use_variables):
    source_use_def_vars = []
    trans_use_def_vars = []
    if s_id in source_stmt_def_variables:
        source_use_def_vars.extend(source_stmt_def_variables[s_id])
    if s_id in source_stmt_use_variables:
        source_use_def_vars.extend(source_stmt_use_variables[s_id])
    for t_id in mapped_trans_stmts:
        if t_id in trans_stmt_def_variables:
            trans_use_def_vars.extend(trans_stmt_def_variables[t_id])
        if t_id in trans_stmt_use_variables:
            trans_use_def_vars.extend(trans_stmt_use_variables[t_id])
    source_use_def_vars = set(source_use_def_vars)
    trans_use_def_vars = set(trans_use_def_vars)
    if source_use_def_vars != trans_use_def_vars:
        return False
    else:
        return True


def check_consecutive_and_truncate(sequence):
    if len(sequence) <= 1:
        return sequence
    ...
    for (x, y, s) in sequence:
        if x_last is None:
            truncated.append((x, y, s))
            x_last = x
            y_last = y
            continue
...
def find_best_sequence(M):
    true_keys = [k for k, v in M.items() if v]
    points = []
    for s in true_keys:
        x, y = s.split('-')
        points.append((int(x), int(y), s))
    ...
    for i in range(n):
        ...
        for j in range(i):
            xj, yj, sj = points[j]
            if xj <= xi and yj <= yi:
                candidate = dp[j] + [points[i]]
                curr = dp[i]

                score_cand = len({p[0] for p in candidate})
                score_curr = len({p[0] for p in curr})
                len_cand = len(candidate)
                len_curr = len(curr)

                if score_cand > score_curr or (score_cand == score_curr and len_cand > len_curr):
                    dp[i] = candidate
    best_seq = []
    best_x_cover = 0
    best_len = 0
    for seq in dp:
        x_cover = len({p[0] for p in seq})
        seq_len = len(seq)
        if x_cover > best_x_cover:
            best_x_cover = x_cover
            best_len = seq_len
            best_seq = seq
        elif x_cover == best_x_cover and seq_len > best_len:
            best_len = seq_len
            best_seq = seq
    best_seq = check_consecutive_and_truncate(best_seq)

    output = {k: False for k in M}
    for k in [item[2] for item in best_seq]:
        output[k] = True
    return output
...
def extend_by_variable(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, source_stmt_list, trans_stmt_list, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables, this_source_trees, this_trans_trees, source_lang, target_lang):
    ...
    s_ret_stmts = []
    for id in unmapped_s_id_seq:
        if source_stmt_list[id].startswith('return_'):
            s_ret_stmts.append(id)
    t_ret_stmts = []
    for id in unmapped_t_id_seq:
        if trans_stmt_list[id].startswith('return_'):
            t_ret_stmts.append(id)
    if len(s_ret_stmts) == len(t_ret_stmts):
        for s_id, t_id in zip(s_ret_stmts, t_ret_stmts):
            extend_new_updated_M[f'{s_id}-{t_id}'] = True
            pass_s_ids.append(s_id)
            pass_t_ids.append(t_id)
    for s_id in unmapped_s_id_seq:
        for t_id in unmapped_t_id_seq:
            ...
            if (s_def_varis == t_def_varis or s_use_varis == t_use_varis) and s_id not in pass_s_ids and t_id not in pass_t_ids:
                if validate_sim_map(['||||'.join(this_source_trees[s_id].getDFS(source_lang)), ['||||'.join(this_trans_trees[t_id].getDFS(target_lang))]], source_lang, target_lang):
                    exis_maps = [k for k, v in extend_new_updated_M.items() if v]
                    if check_insert_valid(exis_maps, f'{s_id}-{t_id}'):
                        extend_new_updated_M[f'{s_id}-{t_id}'] = True
                        pass_s_ids.append(s_id)
                        pass_t_ids.append(t_id)
...
def check_insert_valid(intervals, new_item):
    parsed = []
    for item in intervals:
        x, y = item.split('-')
        parsed.append((int(x), int(y)))
    new_x, new_y = new_item.split('-')
    new_x = int(new_x)
    new_y = int(new_y)
    if new_x > new_y:
        return False
    temp = parsed + [(new_x, new_y)]
    temp_sorted = sorted(temp, key=lambda p: (p[0], p[1]))
    for i in range(1, len(temp_sorted)):
        prev_x, prev_y = temp_sorted[i - 1]
        curr_x, curr_y = temp_sorted[i]
        if curr_x < prev_x or curr_y < prev_y:
            return False
    return True


def best_alignment(list_a, list_b, score_matrix):
    n = len(list_a)
    m = len(list_b)
    ...
    for i in range(n + 1):
        for j in range(m + 1):
            ...
            if i > 0 and j > 0:
                cost = dp[i-1][j-1] + score_matrix[list_a[i-1]][list_b[j-1]]
                if cost < dp[i][j]:
                    dp[i][j] = cost
                    prev[i][j] = (i-1, j-1, 'match')

            if i == n and j > 0:
                cost = dp[i][j-1] + score_matrix[list_a[-1]][list_b[j-1]]
                if cost < dp[i][j]:
                    dp[i][j] = cost
                    prev[i][j] = (i, j-1, 'repeat_a')

            if j == m and i > 0:
                cost = dp[i-1][j] + score_matrix[list_a[i-1]][list_b[-1]]
                if cost < dp[i][j]:
                    dp[i][j] = cost
                    prev[i][j] = (i-1, j, 'repeat_b')
    ...
    while prev[i][j] is not None:
        pi, pj, act = prev[i][j]
        if act == 'match':
            alignment.append((list_a[pi], list_b[pj]))
        elif act == 'repeat_a':
            alignment.append((list_a[-1], list_b[pj]))
        elif act == 'repeat_b':
            alignment.append((list_a[pi], list_b[-1]))
        i, j = pi, pj

    alignment = alignment[::-1]
    return alignment, len(alignment), dp[n][m]


from difflib import SequenceMatcher
def extend_by_sim(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping):
    for k, v in extend_new_updated_M.items():
        if v:
            ids = k.split('-')
            s_id = int(ids[0])
    ...
    for s_id in unmapped_s_id_seq:
        sim_scores[s_id] = {}
        for t_id in unmapped_t_id_seq:
            if source_stmt_list[s_id] in source_stmt2mapping and source_stmt2mapping[source_stmt_list[s_id]] not in [[], [[]], [[], []]]:
                text_s_list = ['||||'.join(item[0].getDFS(source_lang)) for item in
                               source_stmt2mapping[source_stmt_list[s_id]] if item]
            else:
                text_s_list = [source_stmt_list[s_id]]
            if trans_stmt_list[t_id] in trans_stmt2mapping and trans_stmt2mapping[trans_stmt_list[t_id]] not in [[], [[]], [[], []]]:
                text_t_list = ['||||'.join(item[0].getDFS(target_lang)) for item in
                               trans_stmt2mapping[trans_stmt_list[t_id]] if item]
            else:
                text_t_list = [trans_stmt_list[t_id]]
            distances = []
            for text_s in text_s_list:
                text_t = trans_stmt_list[t_id]
                distance = SequenceMatcher(None, text_s, text_t).ratio()
                distances.append(1 - distance)
            for text_t in text_t_list:
                text_s = source_stmt_list[s_id]
                distance = SequenceMatcher(None, text_s, text_t).ratio() / max(len(text_s), len(text_t))
                distances.append(1 - distance)
            if distances:
                sim_scores[s_id][t_id] = sum(distances) / len(distances)
    new_unmapped_s_id_seq = [item for item in unmapped_s_id_seq if item not in pass_s_ids]
    new_unmapped_t_id_seq = [item for item in unmapped_t_id_seq if item not in pass_t_ids]
    if new_unmapped_s_id_seq and new_unmapped_t_id_seq:
        alignment, _, _ = best_alignment(new_unmapped_s_id_seq, new_unmapped_t_id_seq, sim_scores)
        for pair in alignment:
            if validate_sim_map(['||||'.join(this_source_trees[pair[0]].getDFS(source_lang)), ['||||'.join(this_trans_trees[pair[1]].getDFS(target_lang))]], source_lang, target_lang):
                extend_new_updated_M[f'{pair[0]}-{pair[1]}'] = True
    return extend_new_updated_M


def extend_maps(new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables):
    pre_M = {}
    pre_pre_M = {}
    new_new_updated_M = copy.copy(new_updated_M)
    while pre_M != new_new_updated_M:
        if pre_M != {}:
            pre_pre_M = copy.copy(pre_M)
        pre_M = copy.copy(new_new_updated_M)
        extend_new_updated_M = extend_maps_iter1(new_new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables)
        extend_new_updated_M = extend_maps_iter2(extend_new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables)
        new_new_updated_M = find_best_sequence(extend_new_updated_M)
        if new_new_updated_M == pre_M or new_new_updated_M == pre_pre_M:
            new_new_updated_M = copy.deepcopy(extend_new_updated_M)
            break
    return new_new_updated_M


def extend_maps_iter1(new_new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables):
    extend_new_updated_M = copy.deepcopy(new_new_updated_M)
    unmapped_s_ids = []
    for s_stmt_id, s_stmt in enumerate(this_source_trees):
        mapped_stmts = []
        for t_stmt_id, t_stmt in enumerate(this_trans_trees):
            if new_new_updated_M[f'{s_stmt_id}-{t_stmt_id}']:
    ...
    for s_seq_id, unmapped_s_id_seq in enumerate(unmapped_s_id_seqs):
        for t_seq_id, unmapped_t_id_seq in enumerate(unmapped_t_id_seqs):
            if s_seq_id not in passed_s_seq_ids and t_seq_id not in passed_t_seq_ids:
                ...
                if new_new_updated_M[f'{s_pre}-{t_pre}'] and s_fol >= len(this_source_trees) and t_fol >= len(this_trans_trees):
                    extend_new_updated_M = extend_by_variable(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, source_stmt_list, trans_stmt_list, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables, this_source_trees, this_trans_trees, source_lang, target_lang)
                    passed_s_seq_ids.append(s_seq_id)
                    passed_t_seq_ids.append(t_seq_id)
                elif s_fol < len(this_source_trees) and t_fol < len(this_trans_trees) and new_new_updated_M[f'{s_pre}-{t_pre}'] and new_new_updated_M[f'{s_fol}-{t_fol}']:
                    extend_new_updated_M = extend_by_variable(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, source_stmt_list, trans_stmt_list, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables, this_source_trees, this_trans_trees, source_lang, target_lang)
                    passed_s_seq_ids.append(s_seq_id)
                    passed_t_seq_ids.append(t_seq_id)
    return extend_new_updated_M
...
def extend_maps_iter2(new_new_updated_M, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping, source_stmt_def_variables, source_stmt_use_variables, trans_stmt_def_variables, trans_stmt_use_variables):
    extend_new_updated_M = copy.deepcopy(new_new_updated_M)
    unmapped_s_ids = []
    for s_stmt_id, s_stmt in enumerate(this_source_trees):
        mapped_stmts = []
    ...
    for s_seq_id, unmapped_s_id_seq in enumerate(unmapped_s_id_seqs):
        for t_seq_id, unmapped_t_id_seq in enumerate(unmapped_t_id_seqs):
            if s_seq_id not in passed_s_seq_ids and t_seq_id not in passed_t_seq_ids:
                ...
                t_fol = unmapped_t_id_seq[-1]+1
                if new_new_updated_M[f'{s_pre}-{t_pre}'] and s_fol >= len(this_source_trees) and t_fol >= len(this_trans_trees):
                    extend_new_updated_M = extend_by_sim(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping)
                    passed_s_seq_ids.append(s_seq_id)
                    passed_t_seq_ids.append(t_seq_id)
                elif s_fol < len(this_source_trees) and t_fol < len(this_trans_trees) and new_new_updated_M[f'{s_pre}-{t_pre}'] and new_new_updated_M[f'{s_fol}-{t_fol}']:
                    extend_new_updated_M = extend_by_sim(extend_new_updated_M, unmapped_s_id_seq, unmapped_t_id_seq, this_source_trees, this_trans_trees, source_stmt_list, trans_stmt_list, source_lang, target_lang, source_stmt2mapping, trans_stmt2mapping)
                    ...
                    passed_t_seq_ids.append(t_seq_id)
    for s_seq_id, unmapped_s_id_seq in enumerate(unmapped_s_id_seqs):
        if s_seq_id in passed_s_seq_ids:
            continue
        nocomment_unmapped_s_id_seq = [id for id in unmapped_s_id_seq if 'comment' not in source_stmt_list[id]]
        if 0 < len(nocomment_unmapped_s_id_seq) < 4:
            pre = unmapped_s_id_seq[0] - 1
            fol = unmapped_s_id_seq[-1] + 1
            pre_mapped_t = []
            if pre != 0:
                for k, v in extend_new_updated_M.items():
                    if int(k.split('-')[0]) == pre and v:
                        pre_mapped_t.append(int(k.split('-')[1]))
            fol_mapped_t = []
            for k, v in extend_new_updated_M.items():
                if int(k.split('-')[0]) == fol and v:
                    fol_mapped_t.append(int(k.split('-')[1]))
            pre_mapped_t.sort()
            fol_mapped_t.sort()
            if pre == 0:
                for s_id in unmapped_s_id_seq:
                    if validate_sim_map([source_stmt_list[s_id], trans_stmt_list[fol_mapped_t[0]]], source_lang, target_lang):
                        extend_new_updated_M[f'{s_id}-{fol_mapped_t[0]}'] = True
            elif fol >= len(trans_stmt_list):
                for s_id in unmapped_s_id_seq:
                    if validate_sim_map([source_stmt_list[s_id], trans_stmt_list[pre_mapped_t[-1]]], source_lang, target_lang):
                        extend_new_updated_M[f'{s_id}-{pre_mapped_t[-1]}'] = True
            elif pre_mapped_t and fol_mapped_t:
                this_du_variables = []
                for this_id in unmapped_s_id_seq:
                    if this_id in source_stmt_def_variables:
                        this_du_variables.extend(source_stmt_def_variables[this_id])
                    if this_id in source_stmt_use_variables:
                        this_du_variables.extend(source_stmt_use_variables[this_id])
                this_du_variables = set(this_du_variables)
                pre_du_variables = []
                if pre in source_stmt_def_variables:
                    pre_du_variables.extend(source_stmt_def_variables[pre])
                if pre in source_stmt_use_variables:
                    pre_du_variables.extend(source_stmt_use_variables[pre])
                for this_pre_mapped_t in pre_mapped_t:
                    if this_pre_mapped_t in trans_stmt_def_variables:
                        pre_du_variables.extend(trans_stmt_def_variables[this_pre_mapped_t])
                    if this_pre_mapped_t in trans_stmt_use_variables:
                        pre_du_variables.extend(trans_stmt_use_variables[this_pre_mapped_t])
                pre_du_variables = set(pre_du_variables)
                fol_du_variables = []
                if fol in source_stmt_def_variables:
                    fol_du_variables.extend(source_stmt_def_variables[fol])
                if fol in source_stmt_use_variables:
                    fol_du_variables.extend(source_stmt_use_variables[fol])
                for this_fol_mapped_t in fol_mapped_t:
                    if this_fol_mapped_t in trans_stmt_def_variables:
                        fol_du_variables.extend(trans_stmt_def_variables[this_fol_mapped_t])
                    if this_fol_mapped_t in trans_stmt_use_variables:
                        fol_du_variables.extend(trans_stmt_use_variables[this_fol_mapped_t])
                fol_du_variables = set(fol_du_variables)
                if min(len(pre_du_variables), len(this_du_variables)):
                    pre_same_v = len(this_du_variables.intersection(pre_du_variables))/min(len(pre_du_variables), len(this_du_variables))
                else:
                    pre_same_v = len(this_du_variables.intersection(pre_du_variables))
                if min(len(fol_du_variables), len(this_du_variables)):
                    fol_same_v = len(this_du_variables.intersection(fol_du_variables))/min(len(fol_du_variables), len(this_du_variables))
                else:
                    fol_same_v = len(this_du_variables.intersection(fol_du_variables))
                if pre_same_v > fol_same_v:
                    for s_id in unmapped_s_id_seq:
                        if validate_sim_map([source_stmt_list[s_id], trans_stmt_list[pre_mapped_t[-1]]], source_lang, target_lang):
                            extend_new_updated_M[f'{s_id}-{pre_mapped_t[-1]}'] = True
                else:
                    for s_id in unmapped_s_id_seq:
                        if validate_sim_map([source_stmt_list[s_id], trans_stmt_list[fol_mapped_t[0]]], source_lang, target_lang):
                            extend_new_updated_M[f'{s_id}-{fol_mapped_t[0]}'] = True
    for t_seq_id, unmapped_t_id_seq in enumerate(unmapped_t_id_seqs):
        if t_seq_id in passed_t_seq_ids:
            continue
        nocomment_unmapped_t_id_seq = [id for id in unmapped_t_id_seq if 'comment' not in trans_stmt_list[id]]
        if 0 < len(nocomment_unmapped_t_id_seq) < 4:
            pre = unmapped_t_id_seq[0] - 1
            fol = unmapped_t_id_seq[-1] + 1
            pre_mapped_s = []
            if pre != 0:
                for k, v in extend_new_updated_M.items():
                    if int(k.split('-')[1]) == pre and v:
                        pre_mapped_s.append(int(k.split('-')[0]))
            fol_mapped_s = []
            for k, v in extend_new_updated_M.items():
                if int(k.split('-')[1]) == fol and v:
                    fol_mapped_s.append(int(k.split('-')[0]))
            pre_mapped_s.sort()
            fol_mapped_s.sort()
            if pre == 0:
                for t_id in unmapped_t_id_seq:
                    if validate_sim_map([source_stmt_list[fol_mapped_s[0]], trans_stmt_list[t_id]], source_lang, target_lang):
                        extend_new_updated_M[f'{fol_mapped_s[0]}-{t_id}'] = True
            elif fol >= len(trans_stmt_list):
                for t_id in unmapped_t_id_seq:
                    if validate_sim_map([source_stmt_list[pre_mapped_s[-1]], trans_stmt_list[t_id]], source_lang, target_lang):
                        extend_new_updated_M[f'{pre_mapped_s[-1]}-{t_id}'] = True
            elif pre_mapped_s and fol_mapped_s:
                this_du_variables = []
                for this_id in unmapped_t_id_seq:
                    if this_id in trans_stmt_def_variables:
                        this_du_variables.extend(trans_stmt_def_variables[this_id])
                    if this_id in trans_stmt_use_variables:
                        this_du_variables.extend(trans_stmt_use_variables[this_id])
                this_du_variables = set(this_du_variables)
                pre_du_variables = []
                if pre in trans_stmt_def_variables:
                    pre_du_variables.extend(trans_stmt_def_variables[pre])
                if pre in trans_stmt_use_variables:
                    pre_du_variables.extend(trans_stmt_use_variables[pre])
                for this_pre_mapped_s in pre_mapped_s:
                    if this_pre_mapped_s in source_stmt_def_variables:
                        pre_du_variables.extend(source_stmt_def_variables[this_pre_mapped_s])
                    if this_pre_mapped_s in source_stmt_use_variables:
                        pre_du_variables.extend(source_stmt_use_variables[this_pre_mapped_s])
                pre_du_variables = set(pre_du_variables)
                fol_du_variables = []
                if fol in trans_stmt_def_variables:
                    fol_du_variables.extend(trans_stmt_def_variables[fol])
                if fol in trans_stmt_use_variables:
                    fol_du_variables.extend(trans_stmt_use_variables[fol])
                for this_fol_mapped_s in fol_mapped_s:
                    if this_fol_mapped_s in source_stmt_def_variables:
                        fol_du_variables.extend(source_stmt_def_variables[this_fol_mapped_s])
                    if this_fol_mapped_s in source_stmt_use_variables:
                        fol_du_variables.extend(source_stmt_use_variables[this_fol_mapped_s])
                fol_du_variables = set(fol_du_variables)
                if min(len(pre_du_variables), len(this_du_variables)):
                    pre_same_v = len(this_du_variables.intersection(pre_du_variables))/min(len(pre_du_variables), len(this_du_variables))
                else:
                ...
                if pre_same_v > fol_same_v:
                    ...
                    for t_id in unmapped_t_id_seq:
                        if validate_sim_map([source_stmt_list[fol_mapped_s[0]], trans_stmt_list[t_id]], source_lang, target_lang):
                            extend_new_updated_M[f'{fol_mapped_s[0]}-{t_id}'] = True
    closures = find_closure(extend_new_updated_M, source_stmt_list, trans_stmt_list)
    for closure in closures:
        S = list(closure[0])
        F = list(closure[1])
        S.sort()
        ...
        if len(S) == len(F):
            for s_idx, s_id in enumerate(S):
                for f_idx, f_id in enumerate(F):
                    if s_idx != f_idx:
                        extend_new_updated_M[f'{s_id}-{f_id}'] = False
    return extend_new_updated_M
