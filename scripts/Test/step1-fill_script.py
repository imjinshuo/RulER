...
if __name__ == "__main__":
    ...
    for model_name in ['TransCoder', 'TransCoderST', 'Codex', 'Qwen2.5-Coder-32B-Instruct']:
        for source_lang in ['Java', 'Python']:
            ...
            ori_source_func_dir = f'{path_to_code}/{model_name}-data/{source_lang}'
            ori_source_args_dir = f'{path_to_code}/{model_name}-data/{source_lang}-{target_lang}-{target_lang}-args-for-trace'
            ori_source_script_for_trace_dir = f'{path_to_code}/{model_name}-data/{source_lang}-{target_lang}-{source_lang}-script-for-trace'
            ori_source_script_dir = f'{path_to_code}/{model_name}-data/{source_lang}-{target_lang}-{source_lang}-script'
            ori_target_script_dir = f'{path_to_code}/{model_name}-data/{source_lang}-{target_lang}-{target_lang}-script'
            ...
            for ID in IDs:
                ...
                for ID_file in ID_files:
                    ...
                    f = open(f'{ori_target_script_dir}/{ID}.{target_ext}')
                    ...
                    shutil.copy(f'{ori_source_script_dir}/{ID}.{source_ext}', f'{save_dir}/{source_lang}-{target_lang}-{source_lang}-script-for-trace/{ID}.{source_ext}')
